...
test('Bài học 4: Personal notes', async ({ page }) => {
    ...
    await test.step('Add new 10 Personal Notes', async () => {
        ...
        for (let key in personalNotes) {
            // Fill Title
            await page.locator(`//input[@id='note-title']`).fill(personalNotes[key].title);

            // Fill Content
            await page.locator(`//textarea[@id='note-content']`).fill(personalNotes[key].description);
